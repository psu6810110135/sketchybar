...
COVER_CACHE_DIR="${TMPDIR:-/tmp}/sketchybar-${USER}"
COVER_PATH="$COVER_CACHE_DIR/spotify-cover.jpg"
...
update ()
{
  ...
  chmod 700 "$COVER_CACHE_DIR" 2>/dev/null
  ...
  if [ $PLAYING -eq 0 ]; then
    curl -s --max-time 20 --proto '=https' --tlsv1.2 "$COVER" -o "$COVER_PATH"
    if [ "$ARTIST" == "" ]; then
    ...
    args+=(--set spotify.play icon=􀊆
          ...
          --set spotify.cover background.image="$COVER_PATH"
